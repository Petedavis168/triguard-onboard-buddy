...
const EmployeeProfileManagement = () => {
  ...
  const syncWithOnboardingForms = async () => {
    try {
      ...
      // Call the server-side sync function to avoid timeout issues
      const { data, error } = await supabase.rpc('sync_onboarding_to_profiles');
      ...
      const result = data?.[0];
      const profilesCreated = result?.profiles_created || 0;
      ...
      if (profilesCreated === 0) {
        ...
        toast({
          title: "Sync Complete",
          description: `Successfully created ${profilesCreated} rep profile${profilesCreated > 1 ? 's' : ''}`,
        });
      }
