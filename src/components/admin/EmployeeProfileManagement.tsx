...
const EmployeeProfileManagement = () => {
  ...
  const syncWithOnboardingForms = async () => {
    try {
      ...
      // Get all onboarding forms that don't have employee profiles
      ...
      if (onboardingError) {
        console.error('Error fetching onboarding forms:', onboardingError);
      ...
      const existingFormIds = new Set(
        existingProfiles?.filter(p => p.onboarding_form_id).map(p => p.onboarding_form_id) || []
      );
      ...
      const profilesToCreate = formsNeedingProfiles.map(form => {
        const profileData = {
          ...
          email: form.generated_email || form.personal_email || '',
          phone: form.cell_phone || '',
          position: form.employee_role || '',
          hire_date: new Date().toISOString().split('T')[0],
          employee_id: '', // Will be auto-generated by database trigger
          profile_photo_url: form.badge_photo_url || null,
          is_active: ['submitted', 'completed'].includes(form.status)
        };
        
        // Remove empty string values to avoid conflicts
        ...
        // Ensure employee_id is empty string for trigger to work
      ...
      toast({
        title: "Sync Error",
        description: `Failed to sync: ${error.message || 'Unknown error'}`,
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
